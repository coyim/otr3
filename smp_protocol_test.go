...
func TestFullSMPHandshake(t *testing.T) {
	...
	s1, _ := alice.generateSMPStartParameters()
	...
	s2, _ := bob.generateSMPSecondParameters(secret, s1.msg)
	...
	s3, _ := alice.generateSMPThirdParameters(secret, s1, s2.msg)
	...
	// Stores: ???
	// Sends: Rb, cR and D7
	s4, _ := bob.generateSMPFourthParameters(secret, s2, s3.msg)
