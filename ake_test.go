...
func Test_processDHKey(t *testing.T) {
	...
	msg = appendMPI(msg, ake.getTheirPublicValue())
...
func Test_encrypt(t *testing.T) {
	...
	encryptedGx, err := encrypt(ake.r[:], appendMPI(nil, ake.getTheirPublicValue()))
	assertEquals(t, err, nil)
	assertDeepEquals(t, len(encryptedGx), len(appendMPI([]byte{}, ake.getTheirPublicValue())))
}
...
func Test_decrypt(t *testing.T) {
	...
	encryptedGx, _ := encrypt(ake.r[:], appendMPI(nil, ake.getTheirPublicValue()))
	decryptedGx := encryptedGx
	err := decrypt(ake.r[:], decryptedGx, encryptedGx)

	assertEquals(t, err, nil)
	assertDeepEquals(t, decryptedGx, appendMPI([]byte{}, ake.getTheirPublicValue()))
}
...
func Test_generateEncryptedSignature_returnsErrorIfCalcXbFails(t *testing.T) {
	...
	ake.ourKey = bobPrivateKey
	ake.setGXTheir(fixedgx)
	ake.setGYOur(fixedgy)

	_, err := ake.generateEncryptedSignatureTrue(ake.getOurPublicValue(), ake.getTheirPublicValue(), &ake.revealKey)
	assertDeepEquals(t, err, errShortRandomRead)
}
